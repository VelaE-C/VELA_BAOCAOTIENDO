...
async function saveNewTask() {
  ...
  if (!parentId) {
    ...
    // Shift sort_order bằng 1 RPC call duy nhất thay vì loop từng task → nhanh, không conflict
    const { error: shiftErr } = await sb.rpc('shift_sort_order', {
      p_project_id: STATE.currentProject.id,
      p_from_sort:  maxSort
    })
